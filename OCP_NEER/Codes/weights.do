...
*********** Compute share of each reporter in partner total imports
			...
			gen m_i  = trade/(tm) // share of reporter in total imports of each partner
******Total  exports (x)
			bysort reporter :egen x = total(trade) // total exports of each partner 
...
save temp, replace 
***********Import Weights 
			gen weight_m = m_i*(m_mar/(x_mar+m_mar)) if partnercode == 504 // inports weigths 
			...
			egen weightm = rowtotal(weight_m_*) // import weights are replicated from reporter to partner 
			drop weight_m*
			* Expanding the observations 
			gen present = 0 // detecting if a reporter is also present as a partner 
			levelsof partnercode, local(levels) 
			foreach x of local levels { 
				replace present = 1 if reportercode == `x' // if not duplicate values of the import weights to the corresponding partner 
				}
...
use temp, clear
			...
			egen xi = rowtotal(x_i_*) // exports of morocco to all partners entries
			drop x_i_*
			gen x_i = xi/x_mar // share of these exports in total moroccan exports
			/// checking 
...
**********Generating the first term			
			gen alpha = y_t*x_i // the first term of the exports double weights 
********** Compute total imports of each partner 
			gen x_i_k = (m_i)*(Imports) // exports of reporter to partner using ifa data
			bysort partner : egen xc = total(m_i) // needs to sum to 1
			bysort partner : egen cc = total(x_i_k) // needs to sum to 1
			gen pf = Imports - cc
			replace x_i_k = (m_i)*pf +x_i_k
			bysort partner : egen new_cc = total(x_i_k)
			gen new_pf = Imports - new_cc // resampling the values 
			tab new_pf
			// check 
			tab x_i_k if partner == "France" & reporter == "Morocco"
			tab trade if partner == "France" & reporter == "Morocco"
			gen beta = x_i_k/y // the share of partner exports in the supply of the other partner demand 
			replace beta = 0 if beta == .
************ Determine the share of the partner's partner part of moroccan exports 
			 gen sigma = beta*x_i // the second term of double weights 
			 * Sum the partners share in third markets 
			 bysort reporter : egen sum_sigma = total(sigma) // summin g over all the partner's partners
