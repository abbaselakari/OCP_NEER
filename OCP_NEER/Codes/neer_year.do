...
foreach file of local myfiles { 
	merge 1:1 partner year product using  `file', gen(_merge_rate)				// merge the exchange rate data with weights 
	drop if _merge_rate == 2													// drop countries that are not in the exchange data 
    bysort partner product : gen id = _N										// create an id to control for number of years exchange data are present
    drop if id < 20																// drop countries with missung exchange rate data, otherwise will lead to wrong indices
    drop id																		// if for example for country i t+2 is missing while t+1 and t+3 are present 
																				// difference in exchange rates or variations of neer will be taken by the difference between t+3 and t+1 
																				// instead of t+1, t+2 and t+2, t+3. 
*** Generating double weights for n+1 
gen p = 1 if double_weights != .												// we get the number of years for which the country have weights
bysort partner product : egen pp = total(p)										// for those who don't we replicate for the last value 
...
bysort partner product : replace simple_weights = simple_weights_2 if year > x 
	...
	* rescaling to 1 after weights being dropped and added for simple
	...
	drop p q r s e 
	cd "`pdir'/data_derived/neer/index/geometric/year"
	save exchange_year_`file', replace
	project, creates("`pdir'/data_derived/neer/index/geometric/year/exchange_year_`file'") preserve 
